...
//   Kff: Float64Array n×n (row-major, simétrica)
//   rhsList: array de Float64Array(n)  (lados derechos en orden ORIGINAL)
export function factorSolveMany(Kff, n, rhsList) {
  if (n === 0) return { ok: true, uList: rhsList.map(() => new Float64Array(0)), bandwidth: 0 };

  const perm = rcm(n, adjacencyFromDense(Kff, n));
  ...
  // copia de la banda inferior de la matriz permutada
  for (let i = 0; i < n; i++) {
    const pi = perm[i];
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let j = j0; j <= i; j++) {
      ...
      if (i === j) {
        if (s <= 0 || !isFinite(s)) return { ok: false, bandwidth: m };   // no SPD → respaldo
        L[i * w + m] = Math.sqrt(s);
      } else {
  ...
  // resolver cada RHS con la misma factorización
  const uList = [];
  ...
  for (const b of rhsList) {
    ...
    // des-permutar: u[perm[i]] = x[i]
    const u = new Float64Array(n);
    for (let i = 0; i < n; i++) u[perm[i]] = x[i];
    uList.push(u);
  }
  return { ok: true, uList, bandwidth: m };
}
