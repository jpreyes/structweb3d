...
// ──────────────────────────────────────────────────────────────────────────────
import { Model }           from './model/model.js?v=188';
import { Serializer }      from './model/serializer.js?v=188';
import { Viewport }        from './ui/viewport.js?v=188';
import { PropertiesPanel } from './ui/properties.js?v=188';
import { MenuBar }         from './ui/menu.js?v=188';
import { UndoStack }       from './utils/undo.js?v=188';
import { StaticSolver, ensureDefaultLC }   from './solver/static_solver.js?v=188';
import { Results }                         from './solver/postprocess.js?v=188';
import { areaStress, areaBendingStress, vonMises } from './solver/membrane.js?v=188';
import { ModalSolver }                     from './solver/modal_solver.js?v=188';
import { buildNodeIndex, assembleK, assembleF, getNodeDOFs } from './solver/assembler.js?v=188';
import { assembleSparseGlobal, extractFreeCSR } from './solver/sparse.js?v=188';
import { solveNonlinear, solveNonlinearDC } from './solver/nl_lite.js?v=188';
import { solveCorotBeam, corotBeamForceTangent } from './solver/corotbeam.js?v=188';
import { insertInfill } from './model/macromodel.js?v=188';
import { assembleKg } from './solver/geometric.js?v=188';
import { makeFactor } from './solver/linsolve.js?v=188';
import { formFind } from './solver/formfind.js?v=188';
import { ModalResults }                    from './solver/modal_results.js?v=188';
import { modalTimeHistory }                from './solver/timehistory.js?v=188';
import { parseAccelerogram, accStats, scaleToPGA, DEMO_PRESETS, G as GACC } from './solver/accelerograms.js?v=188';
import { SpectrumSolver }                  from './solver/spectrum_solver.js?v=188';
import { StagedSolver }                    from './solver/staged.js?v=188';
import { tendonEquivalentLoads, applyTendon, tendonEcc } from './solver/tendon.js?v=188';
import { buildLane, influenceLine, movingLoadEnvelope, responseReaction, responseSection } from './solver/moving_load.js?v=188';
import { newmarkNonlinear, shearBuilding, rayleighDamping } from './solver/nl_timehistory.js?v=188';
import { checkDrift } from './design/serviceability.js?v=188';
import { seleccionarPerfil, steelCandidates, predimensionar, candidatesForFamily } from './design/autodesign.js?v=188';
import { jointSCWB } from './design/seismic.js?v=188';
import { resolveMaterial } from './design/material_props.js?v=188';
import { resolveSectionProps } from './design/section_props.js?v=188';
import { autoDetectDiaphragms, computeFloorCR, applyDiaphragmConstraints } from './solver/diaphragm.js?v=188';
import { splitElement, splitByLength, discretizeAll, joinElements, intersectarElementos } from './model/discretize.js?v=188';
import { localAxes, stiffnessMatrix, massMatrix, transformMatrix, globalStiffness, applyReleases } from './solver/timoshenko.js?v=188';
import { blockCells, cornerGridIndices } from './model/mesher.js?v=188';
import { coonsGridFromCorners } from './model/mesh_map.js?v=188';
import { meshPolygonIntoModel } from './model/mesh_free.js?v=188';
import { smoothAreasInModel } from './model/mesh_quality.js?v=188';
import { listFormats, exportModel, importModel } from './io/index.js?v=188';
...
class App {
  ...
  _staticWorkerSolve(K, nDOF, freeDOF, Flist, dense = false) {
    return new Promise((resolve, reject) => {
      let worker;
      try { worker = new Worker(new URL('./solver/static_worker.js?v=188', import.meta.url), { type: 'module' }); }
  ...
  async runModal(opts = {}) {
    ...
    try {
      ...
      const modes = await new Promise((resolve, reject) => {
        const worker = new Worker(new URL('./solver/modal_worker.js?v=188', import.meta.url), { type: 'module' });
  ...
  async runTimeHistory(opts = {}) {
    ...
    try {
      ...
      const rawModes = await new Promise((resolve, reject) => {
        const w = new Worker(new URL('./solver/modal_worker.js?v=188', import.meta.url), { type: 'module' });
  ...
  _thSolveInWorker(modes, ag, dt, zeta) {
    return new Promise((resolve, reject) => {
      let w;
      try { w = new Worker(new URL('./solver/timehistory_worker.js?v=188', import.meta.url), { type: 'module' }); }
      catch (e) {
...
4,100</textarea></div>
  ...
  async predimensionarDialog() {
    ...
    if (document.getElementById('pd-assign').checked) {
      const { profileToSection } = await import('./design/profiles.js?v=188');
      this.snapshot();
      ...
      if (!p.profile) {
        const { fromShape } = await import('./design/section_props.js?v=188');
  ...
  _solveNLInWorker(kind, opts) {
    return new Promise((resolve, reject) => {
      let worker;
      try {
        worker = new Worker(new URL('./solver/nl_worker.js?v=188', import.meta.url), { type: 'module' });
      } catch (e) {
  ...
  async runBuckling(opts = {}) {
    ...
    try {
      ...
      const rawModes = await new Promise((resolve, reject) => {
        const worker = new Worker(new URL('./solver/buckling_worker.js?v=188', import.meta.url), { type: 'module' });
...
<details style="margin-top:12px">
  ...
  async aplicarOperacionesModelo(ops, ctx = null) {
    ...
    this.snapshot();
    const { aplicarOperaciones } = await import('./model/model_ops.js?v=188');
  ...
  async _generarDesdeFicha(fichaText) {
    ...
    try {
      ...
      const { generarModelo } = await import('../asistente/generador.js?v=188');
  ...
  importInterop() {
    ...
    const inp = document.createElement('input');
    inp.type = 'file'; inp.accept = '.' + fmts.map(f => f.ext).join(',.') + ',.txt';
    inp.onchange = async () => {
      const file = inp.files[0]; if (!file) return;
      // adivinar el formato por la extensión; si no, preguntar
      const ext = (file.name.split('.').pop() || '').toLowerCase();
      let fmt = fmts.find(f => f.ext === ext);
      if (!fmt) {
        const overlay = document.getElementById('modal-overlay');
        document.getElementById('modal-title').textContent = '🔄 Importar de otro motor';
        document.getElementById('modal-cancel').style.display = '';
        document.getElementById('modal-body').innerHTML = `
          <p style="color:var(--text-muted);font-size:11px;margin:0 0 8px">No se reconoció la extensión «.${ext}». Elige el formato del archivo:</p>
          <div class="prop-field"><label>Formato</label><select id="io-fmt" style="width:100%">${fmts.map(f => `<option value="${f.id}">${f.name}</option>`).join('')}</select></div>`;
        overlay.classList.remove('hidden');
        const okBtn = await new Promise(res => { overlay._resolve = () => res(true); overlay._reject = () => res(false); });
        if (!okBtn) return;
        fmt = fmts.find(f => f.id === document.getElementById('io-fmt').value);
      }
      try {
  ...
  async generarMemoriaDocx() {
    ...
    try {
      const { Docx } = await import('./io/docx.js?v=188');
