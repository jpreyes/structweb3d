...
// ──────────────────────────────────────────────────────────────────────────────
import { Model }           from './model/model.js?v=149';
import { Serializer }      from './model/serializer.js?v=149';
import { Viewport }        from './ui/viewport.js?v=149';
import { PropertiesPanel } from './ui/properties.js?v=149';
import { MenuBar }         from './ui/menu.js?v=149';
import { UndoStack }       from './utils/undo.js?v=149';
import { StaticSolver, ensureDefaultLC }   from './solver/static_solver.js?v=149';
import { Results }                         from './solver/postprocess.js?v=149';
import { ModalSolver }                     from './solver/modal_solver.js?v=149';
import { buildNodeIndex, assembleK, assembleF, getNodeDOFs } from './solver/assembler.js?v=149';
import { assembleSparseGlobal, extractFreeCSR } from './solver/sparse.js?v=149';
import { solveNonlinear, solveNonlinearDC } from './solver/nl_lite.js?v=149';
import { assembleKg } from './solver/geometric.js?v=149';
import { makeFactor } from './solver/linsolve.js?v=149';
import { formFind } from './solver/formfind.js?v=149';
import { ModalResults }                    from './solver/modal_results.js?v=149';
import { modalTimeHistory }                from './solver/timehistory.js?v=149';
import { parseAccelerogram, accStats, scaleToPGA, DEMO_PRESETS, G as GACC } from './solver/accelerograms.js?v=149';
import { SpectrumSolver }                  from './solver/spectrum_solver.js?v=149';
import { StagedSolver }                    from './solver/staged.js?v=149';
import { tendonEquivalentLoads, applyTendon, tendonEcc } from './solver/tendon.js?v=149';
import { buildLane, influenceLine, movingLoadEnvelope, responseReaction, responseSection } from './solver/moving_load.js?v=149';
import { newmarkNonlinear, shearBuilding, rayleighDamping } from './solver/nl_timehistory.js?v=149';
import { checkDrift } from './design/serviceability.js?v=149';
import { seleccionarPerfil, steelCandidates, predimensionar } from './design/autodesign.js?v=149';
import { autoDetectDiaphragms, computeFloorCR, applyDiaphragmConstraints } from './solver/diaphragm.js?v=149';
import { splitElement, splitByLength, discretizeAll, joinElements, intersectarElementos } from './model/discretize.js?v=149';
import { localAxes, stiffnessMatrix, massMatrix, transformMatrix, globalStiffness, applyReleases } from './solver/timoshenko.js?v=149';
import { blockCells, cornerGridIndices } from './model/mesher.js?v=149';
import { coonsGridFromCorners } from './model/mesh_map.js?v=149';
import { meshPolygonIntoModel } from './model/mesh_free.js?v=149';
import { smoothAreasInModel } from './model/mesh_quality.js?v=149';
...
class App {
  ...
  _staticWorkerSolve(K, nDOF, freeDOF, Flist, dense = false) {
    return new Promise((resolve, reject) => {
      let worker;
      try { worker = new Worker(new URL('./solver/static_worker.js?v=149', import.meta.url), { type: 'module' }); }
  ...
  async runModal(opts = {}) {
    ...
    const modalOpts = opts.silent
      ? { nModes: this._lastNModes || 10, method: this._modalMethod || 'subspace', useKg: false }
    ...
    try {
      ...
      const modes = await new Promise((resolve, reject) => {
        const worker = new Worker(new URL('./solver/modal_worker.js?v=149', import.meta.url), { type: 'module' });
  ...
  async runTimeHistory(opts = {}) {
    ...
    try {
      ...
      const rawModes = await new Promise((resolve, reject) => {
        const w = new Worker(new URL('./solver/modal_worker.js?v=149', import.meta.url), { type: 'module' });
  ...
  _thDefaults() {
    ...
    return { dir: d.dir || 'X', zeta: d.zeta ?? 0.05, nModes: d.nModes || 10, ag: demo.a, dt: demo.dt, agName: demo.name };
  }
  ...
  _thSolveInWorker(modes, ag, dt, zeta) {
    return new Promise((resolve, reject) => {
      let w;
      try { w = new Worker(new URL('./solver/timehistory_worker.js?v=149', import.meta.url), { type: 'module' }); }
      catch (e) {
    ...
    return new Promise(resolve => {
      ...
      document.getElementById('modal-body').innerHTML = `
        <div class="prop-row">
          ...
          <div class="prop-field"><label>N° de modos</label><input type="number" id="th-nmodes" value="${d.nModes||10}" min="1" max="40" step="1" style="width:80px"></div>
  ...
  // Panel flotante: curva de respuesta del objeto monitor (nodo/elemento) +
  // animación en el tiempo. El monitor es ELEGIBLE (nodo o elemento + componente).
  _thOpenOverlay() {
    const R = this._thResult;
    ...
    let el = document.getElementById('th-overlay');
    if (!el) { el = document.createElement('div'); el.id = 'th-overlay'; document.body.appendChild(el); }
    ...
    el.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
        <b style="color:var(--accent,#38bdf8)">Respuesta en el tiempo</b>
        <button id="th-close" title="Cerrar" style="background:none;border:none;color:var(--text-muted,#94a3b8);cursor:pointer;font-size:16px;line-height:1">✕</button>
      </div>
      <div style="display:flex;gap:5px;align-items:center;margin-bottom:5px">
        <select id="th-montype" style="font-size:11px"><option value="node">Nodo</option><option value="elem">Elemento</option></select>
        <input type="number" id="th-monid" value="${R.monId}" min="1" step="1" style="width:58px;font-size:11px" title="ID del nodo o elemento a monitorear">
        <select id="th-moncomp" style="font-size:11px;flex:1">${this._thCompOptions('node', R.monComp)}</select>
      </div>
      <div id="th-plot"></div>
    ...
    typeSel.addEventListener('change', () => {
      const sel = this._selElems?.()[0];   // si hay un elemento seleccionado, sugerirlo
      if (typeSel.value === 'elem') { idInp.value = sel || [...this.model.elements.keys()][0] || 1; compSel.innerHTML = this._thCompOptions('elem', 'N'); }
    ...
    el.querySelector('#th-close').addEventListener('click', () => { this._thStopPlay(); el.remove(); this.viewport.clearResults(); });
  ...
  _thExportCSV() {
    ...
    const info = this._thMonitorInfo();
    const col = R.monType === 'elem' ? `${R.monComp}_elem${R.monId}[${info.unit}]` : `${R.monComp}_nodo${R.monId}[${info.unit}]`;
...
4,100</textarea></div>
  ...
  async _autoDesignApply() {
    ...
    const { profileToSection } = await import('./design/profiles.js?v=149');
    this.snapshot();
    let n = 0;
  ...
  async predimensionarDialog() {
    ...
    if (document.getElementById('pd-assign').checked) {
      ...
      if (!p.profile) {
        const { fromShape } = await import('./design/section_props.js?v=149');
  ...
  _solveNLInWorker(kind, opts) {
    return new Promise((resolve, reject) => {
      let worker;
      try {
        worker = new Worker(new URL('./solver/nl_worker.js?v=149', import.meta.url), { type: 'module' });
      } catch (e) {
  ...
  async runBuckling(opts = {}) {
    ...
    try {
      ...
      const rawModes = await new Promise((resolve, reject) => {
        const worker = new Worker(new URL('./solver/buckling_worker.js?v=149', import.meta.url), { type: 'module' });
...
<details style="margin-top:12px">
  ...
  async aplicarOperacionesModelo(ops, ctx = null) {
    ...
    this.snapshot();
    const { aplicarOperaciones } = await import('./model/model_ops.js?v=149');
  ...
  async _generarDesdeFicha(fichaText) {
    ...
    try {
      ...
      const { generarModelo } = await import('../asistente/generador.js?v=149');
  ...
  async generarMemoriaDocx() {
    ...
    try {
      const { Docx } = await import('./io/docx.js?v=149');
