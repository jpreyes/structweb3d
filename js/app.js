...
// ──────────────────────────────────────────────────────────────────────────────
import { Model }           from './model/model.js?v=208';
import { Serializer }      from './model/serializer.js?v=208';
import { Viewport }        from './ui/viewport.js?v=208';
import { PropertiesPanel } from './ui/properties.js?v=208';
import { MenuBar }         from './ui/menu.js?v=208';
import { UndoStack }       from './utils/undo.js?v=208';
import { StaticSolver, ensureDefaultLC }   from './solver/static_solver.js?v=208';
import { Results }                         from './solver/postprocess.js?v=208';
import { areaStress, areaBendingStress, vonMises } from './solver/membrane.js?v=208';
import { ModalSolver, guyanReduce }        from './solver/modal_solver.js?v=208';
import { buildNodeIndex, assembleK, assembleF, getNodeDOFs } from './solver/assembler.js?v=208';
import { assembleSparseGlobal, extractFreeCSR } from './solver/sparse.js?v=208';
import { solveNonlinear, solveNonlinearDC } from './solver/nl_lite.js?v=208';
import { solveCorotBeam, corotBeamForceTangent } from './solver/corotbeam.js?v=208';
import { insertInfill } from './model/macromodel.js?v=208';
import { listMacros, getMacro, insertMacro } from './model/macro_registry.js?v=208';
import { assembleKg } from './solver/geometric.js?v=208';
import { makeFactor } from './solver/linsolve.js?v=208';
import { formFind } from './solver/formfind.js?v=208';
import { ModalResults }                    from './solver/modal_results.js?v=208';
import { modalTimeHistory }                from './solver/timehistory.js?v=208';
import { parseAccelerogram, accStats, scaleToPGA, DEMO_PRESETS, G as GACC } from './solver/accelerograms.js?v=208';
import { SpectrumSolver }                  from './solver/spectrum_solver.js?v=208';
import { StagedSolver }                    from './solver/staged.js?v=208';
import { tendonEquivalentLoads, applyTendon, tendonEcc } from './solver/tendon.js?v=208';
import { buildLane, influenceLine, movingLoadEnvelope, responseReaction, responseSection } from './solver/moving_load.js?v=208';
import { newmarkNonlinear, shearBuilding, rayleighDamping } from './solver/nl_timehistory.js?v=208';
import { checkDrift } from './design/serviceability.js?v=208';
import { seleccionarPerfil, steelCandidates, predimensionar, candidatesForFamily } from './design/autodesign.js?v=208';
import { jointSCWB } from './design/seismic.js?v=208';
import { resolveMaterial } from './design/material_props.js?v=208';
import { resolveSectionProps } from './design/section_props.js?v=208';
import { autoDetectDiaphragms, computeFloorCR, applyDiaphragmConstraints } from './solver/diaphragm.js?v=208';
import { splitElement, splitByLength, discretizeAll, joinElements, intersectarElementos } from './model/discretize.js?v=208';
import { localAxes, stiffnessMatrix, massMatrix, transformMatrix, globalStiffness, applyReleases } from './solver/timoshenko.js?v=208';
import { blockCells, cornerGridIndices } from './model/mesher.js?v=208';
import { coonsGridFromCorners } from './model/mesh_map.js?v=208';
import { meshPolygonIntoModel } from './model/mesh_free.js?v=208';
import { smoothAreasInModel } from './model/mesh_quality.js?v=208';
import { listFormats, exportModel, importModel } from './io/index.js?v=208';
...
class App {
  constructor() {
    ...
    // #103: selector de caso de resultados + descarga del caso actual (en la barra)
  ...
  async mallarRegionLibre(idsArg) {
    ...
    this.toast(`Región mallada: ${res.areaIds.length} celdas (${nQ} quad + ${nT} tri, ${this._behaviorLabel(behavior)}) · h=${h} m`, 'ok');
  }

  // Diálogo de opciones de mallado libre (#52): tamaño/tipo/espesor/comportamiento +
  // controles avanzados del motor (recombinación Blossom, valencia, adaptativo).
  async _mallarRegionDialog(nContorno) {
    const overlay = document.getElementById('modal-overlay');
    document.getElementById('modal-title').textContent = '🔲 Mallar región libre';
    ...
    const upd = () => {
      ...
      // los controles de recombinación sólo aplican a quads
  ...
  // Macromodelo MURO DE RELLENO (#86): 4 nodos de esquina → 2 puntales diagonales
  // solo-compresión (Mainstone). Auto-detecta la rigidez de columna del marco.
  async crearMuroRellenoSeleccion() {
    const ids = this._selNodes();
    ...
    // EcIcol = rigidez a flexión de las COLUMNAS del marco entre las esquinas (verticales).
    let EcIcol = 0, nCol = 0;
    for (const el of this.model.elements.values()) {
      ...
      if (Math.abs(b.z - a.z) > Math.abs(b.x - a.x)) {           // miembro vertical → columna
    ...
    this.viewport.renderModel(this.model);
    this.markDirty(); this._updateStats?.();
    this.toast(`Muro de relleno insertado: puntal w=${(r.strut.w * 100).toFixed(0)} cm (A=${(r.strut.area * 1e4).toFixed(0)} cm²), 2 diagonales solo-compresión. Corra Análisis → No lineal para activar una diagonal.`, 'ok');
  }

  // GENÉRICO (#86): inserta CUALQUIER macromodelo registrado a partir de la selección.
  // Lee el descriptor del registro (`macro_registry.js`), valida nodos/modo, arma el
  // diálogo de parámetros automáticamente y ejecuta su `expand`. Agregar un macromodelo
  // nuevo = registrar su def + (opcional) una entrada de menú que llame a este método.
  async insertMacroFromSelection(id) {
    const def = getMacro(id);
    if (!def) { this.toast(`Macromodelo «${id}» no registrado`, 'warn'); return; }
    if (def.dims && this.model.mode !== def.dims) { this.toast(`«${def.name}» requiere un modelo ${def.dims}`, 'warn'); return; }
    const ids = this._selNodes();
    if (def.nodes && ids.length !== def.nodes) { this.toast(`Seleccione ${def.nodes} nodo(s): ${def.nodesHint || ''}`, 'warn'); return; }
    // Diálogo de parámetros auto-generado desde def.params.
    const overlay = document.getElementById('modal-overlay');
    ...
    this.viewport.renderModel(this.model);
    this.markDirty(); this._updateStats?.();
    this.toast(`Macromodelo «${def.name}» insertado (${(r.strutIds || r.elemIds || []).length} elemento(s) calibrados)`, 'ok');
  }
  ...
  // #105 «Avanzadas»: elegir con checkboxes QUÉ casos estáticos correr.
  async runAnalysisAdvanced() {
    ...
    const overlay = document.getElementById('modal-overlay');
    document.getElementById('modal-title').textContent = '⚙ Elegir casos a correr';
    const rows = statics.map(lc => `
      <label style="display:flex;align-items:center;gap:8px;padding:5px 6px;border:1px solid var(--border);border-radius:6px;margin-bottom:4px;font-size:12px;cursor:pointer">
        ...
        <span style="flex:1">${(lc.name || '').replace(/[<>&]/g, '')}${lc.selfWeight ? ' <span style="color:var(--text-muted)">⊕PP</span>' : ''}</span>
  ...
  // `only` (#105): Set de lcId a resolver (null = todos los casos estáticos).
  runAnalysis(force = false, only = null) {
    this._runOnly = only;   // leído por el filtro de casos más abajo
    ...
    return new Promise(resolve => setTimeout(async () => {
      try {
        ...
        if (!reused) {
          ...
          const only = this._runOnly;   // selección de «Casos…» (#105); null = todos
          ...
          // No cachear corridas PARCIALES (#105): un F5 completo posterior debe recalcular todo.
  ...
  _staticWorkerSolve(K, nDOF, freeDOF, Flist, dense = false) {
    return new Promise((resolve, reject) => {
      let worker;
      try { worker = new Worker(new URL('./solver/static_worker.js?v=208', import.meta.url), { type: 'module' }); }
  ...
  _refreshResultView(autoScale = false) {
    ...
    this._renderResultCaseSelector();   // #103: mantener el selector de caso sincronizado
  ...
  clearResults() {
    ...
    this._pdResult = null;
    this._thResult = null;
    this._thRuns = null;   // #104: olvidar los registros dinámicos corridos
  ...
  exportResults() {
    ...
    this.toast(`Resultados de «${this._resultCaseLabel(this._activeResultKey) || '—'}» exportados (CSV)`, 'ok');
  }

  // #103: etiqueta legible de una clave de _resultsByCase (id numérico = caso; 'C'+id = combo).
  ...
  // Llena el selector de caso de la barra de resultados con lo que YA tiene resultado.
  _renderResultCaseSelector() {
    ...
    sel.innerHTML = [...map.keys()].map(k => `<option value="${k}" ${String(k) === String(active) ? 'selected' : ''}>${(this._resultCaseLabel(k) || k).replace(/[<>&]/g, '')}</option>`).join('');
  }

  // Cambia el caso/combinación mostrado desde la barra de resultados.
  _showResultCase(key) {
    if (key == null) return;
  ...
  async runModal(opts = {}) {
    ...
    try {
      ...
      // ── #5: condensación de Guyan de los GDL SIN MASA antes del worker ───────
      // Evita modos espurios (ρ=0, rotaciones sin inercia). Sólo se activa si hay GDL
      // realmente sin masa; los modelos normales pasan intactos. Se condensa sobre las
      // matrices densas y se re-expanden los modos al volver del worker.
      ...
      let modes = await new Promise((resolve, reject) => {
        const worker = new Worker(new URL('./solver/modal_worker.js?v=208', import.meta.url), { type: 'module' });
  ...
  async runTimeHistory(opts = {}) {
    ...
    try {
      ...
      const rawModes = await new Promise((resolve, reject) => {
        const w = new Worker(new URL('./solver/modal_worker.js?v=208', import.meta.url), { type: 'module' });
      ...
      // #104: recordar cada registro corrido como un «caso» dinámico re-seleccionable.
  ...
  _thSolveInWorker(modes, ag, dt, zeta) {
    return new Promise((resolve, reject) => {
      let w;
      try { w = new Worker(new URL('./solver/timehistory_worker.js?v=208', import.meta.url), { type: 'module' }); }
      catch (e) {
  ...
  _thRenderPanel() {
    ...
    // #104: selector de registro/caso dinámico (si se corrió más de un sismo)
...
4,100</textarea></div>
  ...
  async predimensionarDialog() {
    ...
    if (document.getElementById('pd-assign').checked) {
      const { profileToSection } = await import('./design/profiles.js?v=208');
      this.snapshot();
      ...
      if (!p.profile) {
        const { fromShape } = await import('./design/section_props.js?v=208');
  ...
  _solveNLInWorker(kind, opts) {
    return new Promise((resolve, reject) => {
      let worker;
      try {
        worker = new Worker(new URL('./solver/nl_worker.js?v=208', import.meta.url), { type: 'module' });
      } catch (e) {
  ...
  async runCorotBeam(opts = {}) {
    ...
    this._nlOpenOverlay();
    // Modelo plano → encuadrar la vista FRONTAL X–Z para ver bien la curva (sin
    // zoomExtents después, que resetea a iso). Un frame para que ya esté renderizado.
  ...
  _nlShowStep(s) {
    ...
    if (ro) {
      ...
      if (corot) {
        ro.innerHTML = `λ = <b>${step.lambda.toFixed(3)}</b> · iter ${step.iters} · resid ${step.resid.toExponential(1)}<br>δmáx = ${maxD.toExponential(3)} m · |N|máx = ${Nmax.toFixed(2)} kN · ${step.N.length} elementos`;
      } else {
  ...
  async runBuckling(opts = {}) {
    ...
    try {
      ...
      // Kgff en plano para el worker (+ magnitud del Kg = ¿hay efecto de pandeo?).
      // Vale para barras (Nmax) y para CÁSCARA (tensión de membrana → Kg de áreas).
      ...
      const rawModes = await new Promise((resolve, reject) => {
        const worker = new Worker(new URL('./solver/buckling_worker.js?v=208', import.meta.url), { type: 'module' });
...
<div class="prop-field" style="margin-top:8px">
  ...
  <svg id="sp-graph" viewBox="0 0 420 200" style="width:100%;height:170px;background:var(--bg-elev,#1b1b1b);border:1px solid var(--border);border-radius:6px"></svg>
      ...
      $('sp-case')?.addEventListener('change', (e) => {
        const v = e.target.value;
        if (v === '__new') { $('sp-status') && ($('sp-status').textContent = '✎ Caso nuevo — pulsa «Crear caso» para guardarlo.'); return; }
        ...
        drawGraph();
        const st = $('sp-status'); if (st) { st.textContent = `● Caso «${lc.name}» cargado (guardado). «Aceptar»/«Calcular» guarda los cambios.`; st.style.color = 'var(--ok,#3fb950)'; }
      });
      ...
      // Estado de guardado (#102): aclara si hay cambios sin guardar o si ya se guardó.
      ...
      const createCase = () => {
        ...
        this.toast(`Caso «${lc.name}» creado (sin calcular).`, 'ok');
      };

      // 🗑 Eliminar el caso de espectro seleccionado.
      const deleteCase = async () => {
        const sel = $('sp-case'); const v = sel?.value;
        if (!v || v === '__new') { this.toast('Elige primero un caso guardado para eliminar', 'warn'); return; }
        const lc = this.model.loadCases.get(+v); if (!lc) return;
        const sure = await this._confirm(`¿Eliminar el caso de espectro «${lc.name}»?`);
        if (!sure) return;
        this.snapshot();
        ...
        this.toast(`Caso de espectro «${lc.name}» eliminado`, 'ok');
      };
  ...
  // Etiqueta legible de una categoría de carga (#106).
  _loadTypeLabel(lt) { return LOAD_TYPE_LABEL[lt] || LOAD_TYPE_LABEL.other; }
  // <option>s del selector de categoría de carga, con `sel` marcado.
  ...
  _initResizeHandle() {
    ...
    document.addEventListener('mousemove', e => {
      ...
      // Sólo se cambia el ANCHO DEL PANEL vía la variable CSS --panel-w; así el grid de
      // 5 columnas (incl. la del árbol lateral) lo arma el CSS y no se rompe al arrastrar
      // ni al abrir/cerrar el árbol (antes se fijaba un grid inline de 4 columnas).
  ...
  crearCasosYCombosNorma() {
    const m = this.model;
    this.snapshot();
    // Identificar casos por su CATEGORÍA (#106); como respaldo, por peso propio / nombre.
    ...
    if (!D) D = m.addLoadCase('D — Carga muerta (PP)', true, 'static', null, 'dead');
    let L = byType('live') || findLC(/viva|sobrecarga|live|uso|\bl\b/i);
    if (!L) L = m.addLoadCase('L — Sobrecarga de uso', false, 'static', null, 'live');
...
<details style="margin-top:12px">
  ...
  async aplicarOperacionesModelo(ops, ctx = null) {
    ...
    this.snapshot();
    const { aplicarOperaciones } = await import('./model/model_ops.js?v=208');
  ...
  async _generarDesdeFicha(fichaText) {
    ...
    try {
      ...
      const { generarModelo } = await import('../asistente/generador.js?v=208');
  ...
  // ── Interoperabilidad (#74): exportar/importar a otros motores ───────────────
  async exportInterop() {
    ...
    const overlay = document.getElementById('modal-overlay');
    document.getElementById('modal-title').textContent = '🔄 Exportar a otro motor';
    document.getElementById('modal-cancel').style.display = '';
    document.getElementById('modal-body').innerHTML = `
    ...
    try {
      ...
      if (warnings.length) this._alert('Advertencias al exportar', `<pre style="font-size:11px;color:#d29922;white-space:pre-wrap">${warnings.join('\n')}</pre>`);
    } catch (e) { this.toast('Error al exportar: ' + e.message, 'error'); }
  }
  ...
  async importInterop() {
    ...
    // 1) Elegir PRIMERO el tipo de archivo (el <input type=file> de HTML no permite
    //    filtros con nombre, así que el formato se elige aquí y el archivo se filtra a su ext).
    const overlay = document.getElementById('modal-overlay');
    document.getElementById('modal-title').textContent = '🔄 Importar de otro motor';
    document.getElementById('modal-cancel').style.display = '';
    const opts = fmts.map(f => `<option value="${f.id}">${f.name}</option>`).join('');
    document.getElementById('modal-body').innerHTML = `
    ...
    if (!fmt) return;
    // 2) Abrir el archivo, filtrado a la extensión del formato elegido.
    const inp = document.createElement('input');
    ...
    inp.onchange = async () => {
      ...
      try {
        ...
        this.toast(`Importado de ${fmt.name}: ${model.nodes.size} nodos, ${model.elements.size} barras${warnings.length ? ` (${warnings.length} aviso(s))` : ''}`, warnings.length ? 'warn' : 'ok');
        if (warnings.length) this._alert('Advertencias al importar', `<pre style="font-size:11px;color:#d29922;white-space:pre-wrap">${warnings.slice(0, 30).join('\n')}</pre>`);
      } catch (e) { this.toast('Error al importar: ' + e.message, 'error'); }
    };
    inp.click();
  }

  // ── Importar IFC (#75-#77, G19): ventana flotante con preview side-by-side ───
  async importIFC() {
    const inp = document.createElement('input');
    ...
    inp.onchange = async () => {
      ...
      try {
        mods = await Promise.all([
          import('./io/ifc/ifcToPortico.js?v=208'),
          import('./ui/ifcImportDialog.js?v=208'),
          import('./io/neutral.js?v=208'),
        ]);
      ...
      } catch (e) { this.toast('Error al leer el IFC: ' + e.message, 'error'); return; }
      ...
      try {
        ...
        this.toast(`IFC importado: ${stats.nodes} nodos, ${stats.members} barras${stats.areas ? `, ${stats.areas} áreas` : ''}${allW.length ? ` (${allW.length} aviso(s))` : ''}`, allW.length ? 'warn' : 'ok');
        if (allW.length) this._alert('Advertencias al importar IFC', `<pre style="font-size:11px;color:#d29922;white-space:pre-wrap">${allW.slice(0, 40).join('\n')}</pre>`);
      } catch (e) { this.toast('Error al construir el modelo: ' + e.message, 'error'); }
    };
    inp.click();
  }

  // ── Exportar IFC (#75-#77, G19): modelo → .ifc (IFC4, barras como IfcBeam/Column) ──
  async exportIFC() {
    ...
    try {
      const [{ modelToNeutral }, { neutralToIFC }] = await Promise.all([
        import('./io/neutral.js?v=208'),
        import('./io/ifc/ifcWriter.js?v=208'),
      ]);
      const m = this._predisc ? this.serializer.fromJSON(this._predisc) : this.model;
      ...
      this.toast(`IFC exportado: ${neutral.members.length} barras${neutral.areas?.length ? `, ${neutral.areas.length} áreas` : ''}${warns.length ? ` (${warns.length} aviso(s))` : ''}`, warns.length ? 'warn' : 'ok');
      if (warns.length) this._alert('Advertencias al exportar IFC', `<pre style="font-size:11px;color:#d29922;white-space:pre-wrap">${warns.join('\n')}</pre>`);
    } catch (e) { this.toast('Error al exportar IFC: ' + e.message, 'error'); }
  }
  ...
  async generarBasesCalculo() {
    ...
    // G7: con varios modelos, la memoria los RECORRE (cada uno un capítulo).
    const html = (this._project && this._project.models.length > 1)
      ? await this._memoriaProjectHTML()
      : await (async () => {
          let imgs = { base: null, deformada: null, modos: [] };
          try { imgs = await this._capturarVistasMemoria(); } catch (e) { console.error('Captura de vistas falló:', e); }
          ...
          return this._memoriaHTML(imgs, diseno, deflex, drift);
        })();
  ...
  // G7: memoria HTML que RECORRE todos los modelos del proyecto. Conmuta a cada uno
  // (restaurando sus resultados), arma su sección y al final vuelve al activo original.
  // Cada modelo es un capítulo con salto de página; comparten el <head>/estilos.
  async _memoriaProjectHTML() {
    const orig = this._project.activeId;
    this._stashActiveModel();
    const docs = [];
    for (const entry of this._project.models) {
      this._projectSwitch(entry.id);
      await new Promise(r => setTimeout(r, 80));   // dejar asentar el render para capturar vistas
      ...
      const deflex = this._calcularDeflexionesVigas(diseno?.params);
      const drift  = this._calcularDrift();
      docs.push({ name: entry.name, html: this._memoriaHTML(imgs, diseno, deflex, drift) });
    }
    this._projectSwitch(orig);
    if (!docs.length) return this._memoriaHTML({ base: null, deformada: null, modos: [] }, null, null, null);
    ...
    const sections = docs.map((d, i) => `
      <div style="page-break-before:${i ? 'always' : 'auto'}">
        <h1 style="border-bottom:3px solid #1d4ed8;padding-bottom:6px;margin-top:0">Modelo ${i + 1} · ${String(d.name || '').replace(/[<>&]/g, '')}</h1>
        ${bodyOf(d.html)}
      </div>`).join('\n');
  ...
  async generarMemoriaDocx() {
    // G7: con varios modelos, la memoria Word los RECORRE (un capítulo c/u), igual que la HTML/PDF.
    ...
    try {
      const { Docx } = await import('./io/docx.js?v=208');
      let d;
      if (multi) {
        ...
        const deflex = this._calcularDeflexionesVigas(diseno?.params);
        const drift  = this._calcularDrift();
        d = this._memoriaDocx(Docx, imgs, diseno, deflex, drift);
      }
      this._downloadBlob(d.blob(), multi ? 'memoria_proyecto.docx' : 'memoria_calculo.docx');
      this.toast(`Memoria Word (.docx) descargada${multi ? ` — ${this._project.models.length} modelos` : ''}`, 'ok');
    } catch (e) {
  ...
  // Construye el documento Word de UN modelo: portada + secciones 1–8.
  _memoriaDocx(Docx, imgs, diseno, deflex, drift) {
    const d = new Docx();
    this._memoriaDocxPortada(d);
    d.pageBreak();
    this._memoriaDocxBody(d, imgs, diseno, deflex, drift);
    return d;
  }

  // G7: memoria Word que RECORRE todos los modelos del proyecto (un capítulo c/u,
  // con la misma información que la HTML/PDF). Conmuta a cada modelo para capturar sus
  // vistas y resultados, y al final vuelve al activo original. Espejo de _memoriaProjectHTML.
  async _memoriaProjectDocx(Docx) {
    const d = new Docx();
    const orig = this._project.activeId;
    this._stashActiveModel();
    this._memoriaDocxPortada(d, { nModelos: this._project.models.length });
    let i = 0;
    for (const entry of this._project.models) {
      this._projectSwitch(entry.id);
      await new Promise(r => setTimeout(r, 80));   // dejar asentar el render para capturar vistas
      ...
      const drift  = this._calcularDrift();
      d.pageBreak();
      d.heading(`Modelo ${++i} · ${entry.name || ('Modelo ' + entry.id)}`, 1);
      this._memoriaDocxBody(d, imgs, diseno, deflex, drift);
    }
    this._projectSwitch(orig);
    return d;
  ...
  // Portada de la memoria Word (institución/logo, título y tabla de identificación).
  _memoriaDocxPortada(d, opts = {}) {
    const cm = this._memoria();
    ...
    if (opts.nModelos > 1) d.paragraph([{ text: `Proyecto de ${opts.nModelos} modelos`, color: '0D9488', size: 10 }], { align: 'center' });
    d.spacer();
    d.table(null, [
  ...
  _memoriaDocxBody(d, imgs, diseno, deflex, drift) {
    const m = this.model;
